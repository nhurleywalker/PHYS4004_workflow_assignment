...
### Part 5 - Reporting
1. Create a `.zip` or `.tar` archive in the format LastName_FirstName.zip of the following files:
   - `main.nf`
   - `nextflow.config`
   - `final_report.html`
   - `final_timeline.html`
   - `results/results.csv`
   - `results/*.png`
   - The report requested below (saved as LastName_Firstname.pdf)

1. Please write a report based on the following template
   1. Interpretation: A sentence on what each of the tasks do from 2.2.[i-xi]
   1. Development: A commented code snippet for the two versions of the plot script, one using a for loop, and one using xargs.
   1. Execution: 
   ...
   1. Analysis:
      - Review the `final_report.html` and comment on the resource usage for the three different tasks.
      - What areas of improvement can you identify? What was the most efficient number of cores to use?
      - What changes would you make if the input files were considerably larger?
